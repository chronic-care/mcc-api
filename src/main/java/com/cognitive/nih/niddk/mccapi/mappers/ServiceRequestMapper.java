...
@Slf4j
@Component
public class ServiceRequestMapper implements IServiceRequestMapper {@Override
	
	public ServiceRequestSummary fhir2summary(ServiceRequest in, Context ctx) {
	ServiceRequestSummary out = new ServiceRequestSummary();
...
if (in.hasPerformer())
{
	  ...
	  Practitioner p = ReferenceResolver.findPractitioner(in.getPerformer().get(0),ctx);
	  if (p != null) {
		  if (!p.getName().isEmpty()) {
			  out.setPerformer(p.getName().get(0).getNameAsSingleString());
		  } else {
			  out.setPerformer(in.getPerformer().get(0).getReference());
		  }
	  } else {
		  Patient pat =  ReferenceResolver.findPatient(in.getPerformer().get(0),ctx);
		  if (pat != null) {
			  if (!pat.getName().isEmpty()) {
				  out.setPerformer(pat.getName().get(0).getNameAsSingleString());
			  } else {
				  out.setPerformer(in.getPerformer().get(0).getReference());
			  }
		  } else {
